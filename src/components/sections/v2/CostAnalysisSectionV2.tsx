...
export default function CostAnalysisSectionV2() {
  ...
  return (
    <section id="cost-analysis-section" className="section-padding bg-gradient-to-br from-neutral-50 to-white">
      <div className="section-container">
        ...
        <motion.div 
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-neutral-900 mb-6 font-sans leading-tight">
          ...
          >
            ...
            <div className="space-y-4 mb-8">
              {wastefulExpenses.map((expense, index) => (
                <motion.div
                  key={index}
                  className={`flex flex-col sm:flex-row sm:items-center sm:justify-between p-4 rounded-2xl ${expense.bgColor} border border-white/60 gap-2`}
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className={`w-3 h-3 rounded-full mt-1 flex-shrink-0 ${expense.color.replace('text-', 'bg-')}`}></div>
                    <span className="text-neutral-800 font-medium font-sans text-sm sm:text-base leading-snug">{expense.item}</span>
                  </div>
                  <span className={`font-bold text-xl ${expense.color} font-sans ml-6 sm:ml-0`}>{expense.price}</span>
                </motion.div>
              ))}
            </div>
            ...
            <motion.div 
              ...
              viewport={{ once: true }}
            >
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 text-xl sm:text-2xl font-bold">
                <span className="text-neutral-900 font-sans">Total desperdiçado:</span>
                <span className="text-red-600 font-sans text-2xl sm:text-2xl">R$ {totalWaste.toLocaleString()}</span>
              </div>
            </motion.div>
          </motion.div>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            <div className="text-center mb-6">
              <h3 className="text-xl md:text-2xl lg:text-3xl font-bold text-neutral-900 mb-4 font-sans leading-tight">
        ...
        >
          <div className="bg-gradient-to-r from-primary-500 to-secondary-500 text-white rounded-3xl p-8 max-w-3xl mx-auto">
            ...
            <p className="text-xl font-semibold font-sans mb-6">
              Não é sobre gastar menos. É sobre <strong>gastar melhor</strong> — com consciência e estratégia.
            </p>
            <button 
              onClick={() => scrollToElement('story-section')}
              className="bg-white text-primary-600 px-8 py-3 rounded-full font-bold hover:scale-105 transition-transform duration-200"
            >
              Conheça quem criou a solução
            </button>
          </div>
        </motion.div>
      </div>
